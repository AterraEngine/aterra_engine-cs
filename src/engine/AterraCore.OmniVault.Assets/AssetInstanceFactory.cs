...
public class AssetInstanceFactory(ILogger logger) : IAssetInstanceFactory {
    ...
    public bool TryCreate<T>(AssetRegistration registration, Ulid predefinedUlid, [NotNullWhen(true)] out T? assetInstance) where T : class, IAssetInstance {
        ...
        try {
            TConstructor constructorDelegate = _constructorCache.GetOrAdd(registration.Type, _ => CreateConstructorDelegate(registration.Constructor));
            TActionsArray parameterActions = _actionsMap.GetOrAdd(registration.AssetId, _ => CreateParameterActions(registration));
            ...
            ParameterPool.Return(parameters, clearArray: true);
        }
    }
