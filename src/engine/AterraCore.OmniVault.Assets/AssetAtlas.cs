...
public class AssetAtlas : IAssetAtlas {
    public FrozenDictionary<AssetId, AssetRegistration> AssetsById { get; internal set; } = null!;
    public FrozenDictionary<Type, AssetId> AssetsByType { get; internal set; } = null!;

    public FrozenDictionary<CoreTags, FrozenSet<AssetId>> CoreTaggedAssets  { get; internal set; } = null!;
    public FrozenDictionary<string, FrozenSet<AssetId>> StringTaggedAssets  { get; internal set; } = null!;
    ...
    public bool TryGetAssetType(AssetId assetId, [NotNullWhen(true)] out Type? type) {
        type = default;
        if (!AssetsById.TryGetValue(assetId, out AssetRegistration registration)) {
            return false;
        }
