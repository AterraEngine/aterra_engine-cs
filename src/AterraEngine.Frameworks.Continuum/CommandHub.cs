...
// Code
// ---------------------------------------------------------------------------------------------------------------------
public class CommandHub<TCommand, TOutput> : ICommandHub where TCommand : ICommand<TOutput> where TOutput : struct {
    ...
    private ICommandHandler<TCommand, TOutput>? Subscriber { get; set; } = null;
    ...
    public async ValueTask<T1> PublishAsync<T0,T1>(T0 commandData) where T0 : ICommand<T1> where T1 : struct {
        ...
        await _channel.Writer.WriteAsync((typedCommand, _replyChannel));
        while (await _replyChannel.Reader.WaitToReadAsync()) {
